...
def load_sst2_dataset(data_num): 
    print('Getting Data...')
    dataset = []
    path = './data/sst2/test_{}.csv'.format(data_num)
    csvfile = open(path, 'r')
...
def load_yelp_dataset(data_num=100):
    print("Getting yelp dataset...")
    dataset  = []
    doc_count = 0  # number of input sentences
    path = './data/yelp/test_{}.csv'.format(data_num)
    print(f"load yelp file from:{path}")
    csvfile = open(path, 'r')
    for line in csv.reader(csvfile, delimiter=',', quotechar='"'):
        text = line[0]
        label = int(line[1])
...
def create_save_path(args):
    ...
    attacker_log_path = os.path.join(os.getcwd(), "save_results/attacks",\
    "{}_{}".format(args.dataset,args.model_type))
...
def attack(args):
    ...
    if args.dataset == 'sst2':
        ...
        dataset = load_sst2_dataset(args.attack_num)
        ...
        dataset = load_yelp_dataset(args.attack_num)
