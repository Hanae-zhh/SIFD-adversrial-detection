...
def build_trainlog(args, **kwargs):
    ...
    logging_file_path  = os.path.join(args.save_path, \
        '{}_{}_{}.log'.format(args.mode,args.dataset,args.train_mode))
...
def get_sst_data(args,type='train'):
    ...
    path = './data/{}/'.format(args.dataset) + type + '.tsv'
    count = 0
...
def get_imdb_data(args,type= 'train'):
    texts = []
    labels = []
    input_file = os.path.join("/data/zhanghData/Datasets/imdb","{}.txt".format(type))
...
def read_agnews_mini_files(filetype):
    texts = []
    ...
    path = r'/data/zhangh/Datasets/agnews_mini_csv/{}.csv'.format(filetype)
    csvfile = open(path, 'r')
    for line in csv.reader(csvfile, delimiter=',', quotechar='"',quoting=csv.QUOTE_MINIMAL):
        #content = line[1] + ". " + line[2]
        content = line[2] # delete title
        texts.append(content)
        labels_index.append(int(line[0]))
        doc_count += 1
...
def get_agnews_files(type):
    ...
    path = r'/data/zhanghData/Datasets/ag_news_csv/{}.csv'.format(type)
    logger.info("load dateset from: {path}")
    csvfile = open(path, 'r')
...
def run_train_nn_v1(args):
    ...
    train_dataloader = make_dataloader(data_module.train_texts,data_module.train_pols,data_module.word_to_idx)
    val_dataloader = make_dataloader(data_module.val_texts,data_module.val_pols,data_module.word_to_idx)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            print(
                "Epoch {}, batch {}/{}: loss {}".format(
                    epoch, step, len(train_dataloader), loss.item()))
        ...
        acc = flat_accuracy(np.array(predictions), np.array(data_module.val_pols))
...
def run_train_nn(args):
    ...
    for epoch in range(1,args.epochs+1):
        ...
        #     best_epoch = (epoch, total_loss)
        if eval_acc < best_epoch[1]:
            best_epoch = (epoch, eval_acc)
...
def run_train_bert(args):
    ...
    best_epoch = 0
    best_val = 0.0
    if train_mode =='freelb':
        adv_trainer = FreeLB(adv_K=args.adv_K, adv_lr=args.adv_lr, #advk=3
        ...
        #model, emb_name, epsilon=1., alpha=0.3):
        adv_trainer = PGD(model, emb_name='word_embeddings', epsilon=args.pgd_epsilon,alpha= args.pgd_alpha)
    ...
    for epoch in range(epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            if train_mode =='freelb':
                ...
                adv_trainer.backup_grad()  #克隆embedding参数
                # 对抗训练
                for t in range(args.pgd_k):
                    ...
                    if t != args.pgd_k-1:
                        #print("Here:[t != args.pgd_k-1]")
                        model.zero_grad()
                    else:
                        #print("else")
                        adv_trainer.restore_grad()
                    loss_adv , logits_adv = model(**inputs)[:2]
                    loss_adv.backward() # 反向传播，并在正常的grad基础上，累加对抗训练的梯度
                adv_trainer.restore() # 恢复embedding参数
...
# CUDA_VISIBLE_DEVICES=3 python3 run_train.py --do_train --pgd --adv_K 10 --fada --base_model bert --epochs 10 --batch_size 64 --save_path ./saved_models/SST2_model/pgd_temp/ 

# CUDA_VISIBLE_DEVICES=5 python3 run_train.py --dataset imdb --mode train --train_mode freelb  --model_type roberta

# CUDA_VISIBLE_DEVICES=2 python3 run_train.py --dataset imdb --mode train --model_type roberta --train_mode freelb

# # CUDA_VISIBLE_DEVICES=2 python3 run_train.py --dataset agnews --mode train --model_type lstm --train_mode cnn
    args = Args()
    logger = build_trainlog(args)
    logger.info("---args info---")
    logger.info(f"batch_size:{args.batch_size}, epochs:{args.epochs}")
    # logger.info("fgm_epsilon:{}\n adv_lr:{}\nadv_K:{}\nadv_init_mag:{}\nadv_norm_type:{}\n\
    #     adv_max_norm:{}\nhidden_dropout_prob:{}\nattention_probs_dropout_prob:{}\ngradient_accumulation_steps:{}".\
    #     format(args.fgm_epsilon,args.adv_lr,args.adv_K,args.adv_init_mag,args.adv_norm_type,\
    #     args.adv_max_norm,args.hidden_dropout_prob,args.attention_probs_dropout_prob,args.gradient_accumulation_steps))
