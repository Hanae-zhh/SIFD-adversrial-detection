...
from utils.data_utils import clean_str, cut_raw, save_pkl, load_pkl, shuffle_lists
...
class DataModule:
    def __init__(self, config, logger,gpu):
        ...
        self.logger.info("Initialize DataModule")
        ...
        if self.config.mode == "train":
            ...
            self.logger.info("Save data for restore")
            self.save_data()

        else:
            self.logger.info("Load existing data")
            self.load_data()

        if self.config.mode == "train":
            self.print_data_stats()

        if self.config.mode == "detect" or self.config.mode == "vote":
            self.embeddings = []
            self.dist_mat = []
            ...
            # print(len(self.idx_to_dist_mat_idx))
            # print(len(self.dist_mat_idx_to_idx))
            # print(self.idx_to_dist_mat_idx)
            # print(self.dist_mat_idx_to_idx)
    ...
    def prepare_init_embs(self):
        ...
        if not os.path.exists(self.config.path_to_pre_trained_init):
            self.logger.info("Compute pre-trained init embs")
            self.logger.info(
            ...
            self.logger.info("Done. {} words loaded.".format(len(model)))
            ...
            self.init_pretrained = np.load(self.config.path_to_pre_trained_init)

    def load_LM(self):
        self.logger.info("Load GPT-2 language model")
    ...
    def populate_embeddings(self):
        self.logger.info("Use pre-trained embeddings")

        if not os.path.exists(self.config.path_to_dist_mat):
            self.logger.info("Prepare dist mat")
            self.prepare_embeddings()
        else:
            self.logger.info(
                "Load embeddings from {}".format(self.config.path_to_dist_mat)
            )
            self.dist_mat = np.load(self.config.path_to_dist_mat)
    ...
    def save_data(self):
        base_path = os.path.join(self.config.pre_trained_base,'data')
    ...
    def load_data(self):
        base_path = "{}/data".format(self.config.pre_trained_base)
        self.logger.info("Load saved data from {}".format(base_path))
        ...
        print("self.test_texts: {}/{}".format(base_path, "test_texts.pkl"))
        ...
        self.logger.info("Vocab size: {}".format(len(self.vocab)))

    def prepare_embeddings(self):
        """
        Source: https://github.com/nesl/nlp_adversarial_examples (modified)
        """
        self.logger.info("Loading {} Model".format("glove"))
        f = open(self.config.glove_path, "r")
        model = {}
        ...
        self.logger.info("Dist mat shape {}".format(np.shape(self.dist_mat)))
        self.logger.info("Save mat")
        ...
        np.save(self.config.path_to_dist_mat, self.dist_mat)

        self.logger.info("Saved mat")
    ...
    def print_data_stats(self):
        ...
        self.logger.info("Val size: {} ({})".format(len(self.val_texts), val_dist))
    ...
    def populate_imdb_1(self):
        ...
        for (mode, texts, pols, raw_texts) in splits:
            ...
            path = os.path.join(self.config.path_to_imdb,"{}.txt".format(mode))
            self.logger.info("load dateset from: {}".format(path))
            with open(path, "r", encoding='utf-8') as f:
                count = 1
                for line in f:
                    data = line.strip().split('\t')
                    raw_texts.append(data[0])#存放原始数据
                    texts.append(#存放预处理后的数据
                        cut_raw(
                            clean_str(data[0], tokenizer=self.spacy_tokenizer),
                            self.config.max_length,
                        )
                    )
                    pols.append(1 if data[1]=='1' else 0)
                    count += 1
                    # if count >1000:
                    #     break
        ...
        val_thr = int((1-self.config.val_split_size)*len(self.train_texts))
    ...
    def populate_imdb(self):
        ...
        val_thr = int(len(self.train_texts) - self.config.val_split_size)
    ...
    def populate_yelp(self):
        splits = [
            ("train", self.train_texts, self.train_pols, self.train_raw),
            ("test", self.test_texts, self.test_pols, self.test_raw),
        ]
        # path = self.config.path_to_agnews
        # csvfile = open(path, 'r')
        for (mode, texts, pols, raw_texts) in splits:
            path = os.path.join(self.config.path_to_yelp,"{}.csv".format(mode))
            self.logger.info("load dateset from: {}".format(path))
            with open(path, "r") as f:
                ...
                for line in reader:
                    if line[1]=='label':
                        continue
                    raw_texts.append(line[0])#存放原始数据
                    texts.append(#存放预处理后的数据
                        cut_raw(
                            clean_str(line[0], tokenizer=self.spacy_tokenizer),
                            self.config.max_length,
                        )
                    )
                    pols.append(1 if line[1]=='2' else 0)
    ...
    def populate_agnews(self):
        ...
        # csvfile = open(path, 'r')
        for (mode, texts, pols, raw_texts) in splits:
            path = os.path.join(self.config.path_to_agnews,"{}.csv".format(mode))
            self.logger.info("load dateset from: {}".format(path))
            with open(path, "r") as f:
                for line in csv.reader(f, delimiter=',', quotechar='"'):
                    raw_texts.append(line[2])#存放原始数据
                    texts.append(#存放预处理后的数据
                        cut_raw(
                            clean_str(line[2], tokenizer=self.spacy_tokenizer),
                            self.config.max_length,
                        )
                    )
                    pols.append(int(line[0])-1)
    ...
    def populate_sst2(self):
        splits = [
            ("train", self.train_texts, self.train_pols, self.train_raw),
            ("dev", self.test_texts, self.test_pols, self.test_raw),
        ]

        for (split, texts, labels, texts_raw) in splits:
            path = os.path.join(self.config.path_to_sst2,"{}.tsv".format(split))
